...
def _collect_pss_eligible_ip_surg(
    outclaims: DataFrame, ref_table: DataFrame
) -> DataFrame:
    ...
    df_proc_pivot = (
        ...
        .filter(spark_funcs.col("icd_proc").isNotNull())
    )

    proc_w_ccs = df_proc_pivot.join(
        ref_table, on=(ref_table.code == df_proc_pivot.icd_proc), how="inner"
    ).select(
        "caseadmitid",
        "sequencenumber",
        "member_id",
...
def _collect_pss_eligible_op_surg(
    ...
    hcpcs_w_ccs = (
        outclaims_filter.join(
            ref_table, on=(outclaims_filter.hcpcs == ref_table.code), how="inner"
        )
        .select(
            "member_id",
            "caseadmitid",
...
def _flag_elig_drgs(
    outclaims: DataFrame, inpatient_pss: DataFrame, ref_table: DataFrame
) -> DataFrame:

    inpatient_pss_drg = (
        inpatient_pss.join(outclaims, on="sequencenumber", how="inner")
        .select(
        ...
        .join(ref_table, on="ccs", how="inner")
        .where(spark_funcs.col("prm_drg") == spark_funcs.col("code"))
        .select(
            "member_id",
            "caseadmitid",
            "sequencenumber",
...
def _flag_er_directed(outclaims: DataFrame, pss_claims: DataFrame) -> DataFrame:
    ...
    ed_claims = (
        outclaims.select(
            ...
            spark_funcs.when(
                spark_funcs.col("hcpcs").isin(ER_HCPCS), spark_funcs.lit("Y")
            )
            .when(spark_funcs.col("revcode").isin(ER_REV), spark_funcs.lit("Y"))
            .otherwise(spark_funcs.lit("N"))
            .alias("er_flag"),
        )
        .where(spark_funcs.col("er_flag") == "Y")
        .distinct()
    )

    pss_claims_w_er = (
        pss_claims.join(ed_claims, on="member_id", how="left_outer")
        .where(
            spark_funcs.col("ed_date").between(
                spark_funcs.date_sub(spark_funcs.col("prm_fromdate"), 1),
    ...
    pss_claims_er_directed = pss_claims.join(
        pss_claims_w_er, on=["sequencenumber", "ccs", "position"], how="left_outer"
    ).select(
        ...
        pss_claims.ccs,
        pss_claims.position,
        spark_funcs.when(spark_funcs.col("er_directed").isNull(), spark_funcs.lit("N"))
        .otherwise(spark_funcs.lit("Y"))
        .alias("er_directed"),
    )
...
def _flag_acute_transfer(outclaims: DataFrame, pss_claims: DataFrame) -> DataFrame:

    acute_transfers = (
        outclaims.select(
            "member_id", spark_funcs.col("prm_todate").alias("transfer_date")
        )
        .where(spark_funcs.col("prm_acute_transfer_to_acute_yn") == "Y")
        .distinct()
    )

    pss_claims_w_transfer = (
        pss_claims.join(acute_transfers, on="member_id", how="left_outer")
        .where(
    ...
    pss_claims_transfer = pss_claims.join(
        ...
        pss_claims.ccs,
        pss_claims.position,
        spark_funcs.when(spark_funcs.col("transfer_yn").isNull(), spark_funcs.lit("N"))
        .otherwise(spark_funcs.lit("Y"))
        .alias("transfer_yn"),
    )
...
def _ip_dupe_filter(outclaims: "DataFrame", ip_pss: "DataFrame") -> DataFrame:

    caseadmit_window = (
        Window()
        .partitionBy("caseadmitid")
        .orderBy("position", spark_funcs.desc("mr_allowed"), "prm_fromdate")
    )

    ip_w_allowed = ip_pss.join(outclaims, on="sequencenumber", how="inner").select(
        ip_pss.member_id,
        ip_pss.caseadmitid,
    ...
    ip_pss_ranked = (
        ...
        .where(spark_funcs.col("order") == 1)
        .drop("order", "mr_allowed")
    )

    return ip_pss_ranked


def _op_dupe_filter(outclaims: "DataFrame", op_pss: "DataFrame") -> DataFrame:

    fromdate_window = (
        Window()
        .partitionBy("member_id", "prm_fromdate")
        .orderBy(spark_funcs.desc("paiddate"), spark_funcs.desc("mr_allowed"))
    )

    op_pss_w_paiddate = op_pss.join(outclaims, on="sequencenumber", how="inner").select(
        op_pss.member_id,
        op_pss.caseadmitid,
...
def _calc_ip_pss(
    ...
    inpatient_surgery = _collect_pss_eligible_ip_surg(
        outclaims=dfs_input["outclaims"], ref_table=dfs_refs["icd_procs"]
    )
    ...
    inpatient_transfer_directed = _flag_acute_transfer(
        outclaims=dfs_input["outclaims"], pss_claims=inpatient_drg_filter
    )
    ...
    inpatient_surgery_flagged = (
        inpatient_transfer_directed.join(
            inpatient_er_directed, on=["sequencenumber", "ccs", "position"], how="inner"
        )
        .withColumn(
            "ccs_preventable_yn",
            spark_funcs.when(
                spark_funcs.col("transfer_yn") == "Y", spark_funcs.lit("N")
            )
            .when(spark_funcs.col("er_directed") == "Y", spark_funcs.lit("N"))
            .otherwise(spark_funcs.lit("Y")),
        )
        .select(
    ...
    ip_pss_final = _ip_dupe_filter(
        outclaims=dfs_input["outclaims"], ip_pss=inpatient_surgery_flagged
    )
...
def _calc_op_pss(
    ...
    outpatient_surgery = _collect_pss_eligible_op_surg(
        outclaims=dfs_input["outclaims"], ref_table=dfs_refs["hcpcs"]
    )

    outpatient_er_directed = _flag_er_directed(
        outclaims=dfs_input["outclaims"], pss_claims=outpatient_surgery
    )

    outpatient_ip_filter = (
        outpatient_er_directed.where(~spark_funcs.col("ccs").isin(IP_ONLY_CCS))
        .withColumn(
            "ccs_preventable_yn",
            spark_funcs.when(
                spark_funcs.col("er_directed") == "Y", spark_funcs.lit("N")
            ).otherwise(spark_funcs.lit("Y")),
        )
        .select(
            "member_id",
    ...
    op_pss_final = _op_dupe_filter(
        outclaims=dfs_input["outclaims"], op_pss=outpatient_ip_filter
    )
...
def calculate_pss_decorator(
    ...
    **kwargs,
    ...
    inpatient_pss = _calc_ip_pss(dfs_input=dfs_input, dfs_refs=dfs_refs)

    outpatient_pss = _calc_op_pss(dfs_input=dfs_input, dfs_refs=dfs_refs)

    op_ip_pss = inpatient_pss.union(outpatient_pss)

    ccs_eligible_w_flags = (
        dfs_input["outclaims"]
        .select("sequencenumber", "mr_line_case")
        .withColumn(
            "ccs_eligible_yn",
            spark_funcs.when(
                spark_funcs.col("mr_line_case").startswith("I12"), spark_funcs.lit("Y")
            )
            .when(
                spark_funcs.col("mr_line_case").startswith("O12"), spark_funcs.lit("Y")
            )
            .otherwise(spark_funcs.lit("N")),
        )
    )
    ...
    ccs_calc = ccs_eligible_w_flags.join(
        op_ip_pss, on="sequencenumber", how="left_outer"
    ).select(
...
class PSSDecorator(ClaimDecorator):
    ...
    def _calc_decorator(
        ...
        **kwargs,
        ...
        return calculate_pss_decorator(dfs_input, dfs_refs, **kwargs)
